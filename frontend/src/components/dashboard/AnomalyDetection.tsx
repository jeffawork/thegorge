...
interface Anomaly {
  id: string
  type: 'response_time' | 'error_rate' | 'throughput' | 'unusual_pattern' | 'security'
...
export const AnomalyDetection: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'critical' | 'high'>('all')
      ...
      const mockAnomalies: Anomaly[] = [
        {
          ...
          description: 'Average response time increased by 300% compared to baseline',
        ...
        },
        {
          ...
          description: 'Detected unusual request pattern with 80% GET requests vs normal 60%',
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            {viewMode === 'list' ? <BarChart3 className="w-4 h-4" /> : <PieChart className="w-4 h-4" />}
          </button>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 bg-purple-500/10 border border-purple-500/30 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              <Brain className="w-5 h-5 text-purple-400" />
              <span className="text-sm font-medium text-gray-300">Total Anomalies</span>
            </div>
            <div className="text-2xl font-bold text-purple-400">{stats.totalAnomalies}</div>
          </div>
          ...
          <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
            ...
            <div className="text-2xl font-bold text-red-400">{stats.activeAnomalies}</div>
          </div>
          ...
          <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
            ...
            <div className="text-2xl font-bold text-green-400">{stats.detectionAccuracy}%</div>
          </div>
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-white font-semibold">ML Model: Active</span>
          </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-white font-semibold">{anomaly.confidence}%</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Deviation:</span>
                    <span className={`ml-2 font-semibold ${getDeviationColor(anomaly.deviation)}`}>
                      {anomaly.deviation > 0 ? '+' : ''}{anomaly.deviation.toFixed(1)}%
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-400">Expected:</span>
                    <span className="ml-2 text-white">{anomaly.expectedValue}</span>
                  </div>
                  <div>
                    <span className="text-gray-400">Actual:</span>
                    <span className="ml-2 text-white">{anomaly.actualValue}</span>
                  </div>
                </div>
      ...
      <div className="bg-black/20 rounded-lg p-6">
        <h5 className="text-lg font-semibold text-white mb-4">Model Performance</h5>
